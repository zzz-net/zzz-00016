...
- 风险规则校验：admin 可维护禁用站点/禁行时间段/车辆限制/线路关键词四类规则，全部落 SQLite 重启不丢；在提交、复制再提交、发布前自动拦截命中，返回稳定错误码 `RISK_RULE_VIOLATION` 及详情，不写入错误审批流；规则变更、命中、解除写入 `audit_logs`；admin/调度/安全员可导出（JSON/CSV，含状态与命中次数），admin 可批量导入；权限严格分级（teacher 仅见本人被拦截原因）
- 改线公告通知：已发布的改线申请可由 admin 生成可追踪版本化公告（含线路、影响站点、生效时间、备注）；调度/安全员/admin 可查看全部公告；普通老师仅能看到本人提交申请相关的公告（响应不暴露内部配置、后台筛选条件或其他老师信息）；公告落 SQLite，重启可查；同一申请+版本不能重复生成，重复请求返回稳定错误码 `ANNOUNCEMENT_DUPLICATE` 及现有公告摘要；支持 JSON/CSV 导出（按角色隔离），每次导出写入 `audit_logs`
